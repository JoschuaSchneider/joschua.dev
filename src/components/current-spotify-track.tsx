    ...
    <div className="relative flex items-center p-4 rounded-md bg-gray-50 dark:bg-gray-900">
      {data && data.item && data.is_playing && (
        <>
          ...
          <div className="flex flex-col justify-center ml-4">
            ...
            <Text>{data.item.artists.map(artist => artist.name).join(", ")}</Text>
          </div>
        </>
      )}
      {!data?.is_playing && (
      ...
      {data?.is_playing ? (
        <svg
          ...
          className={'ml-auto mr-4 text-green-500 animate-spin duration-[4s]'}
          viewBox="0 0 400 400"
          version="1"
        >
          <defs>
...
export default CurrentSpotifyTrack
